...
export async function onRequestGet(context){
 ...
 try{
  ...
  const response=await fetch(endpoint,{headers:{Accept:'application/json','User-Agent':'AsteriaxVerse/1.0'}});if(!response.ok)throw new Error(`Traduction ${response.status}`);
  const payload=await response.json(),translated=Array.isArray(payload?.[0])?payload[0].map(part=>Array.isArray(part)?part[0]:'').filter(Boolean).join(' ').trim():'';if(!translated)throw new Error('Réponse de traduction vide');
