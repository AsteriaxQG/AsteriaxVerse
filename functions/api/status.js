...
async function fetchText(url){
  ...
  const r=await fetch(fresh,{headers:{Accept:'text/html,application/xhtml+xml','Accept-Language':'en-US,en;q=0.9','User-Agent':'AsteriaxVerse/2.2'},cache:'no-store'});
...
function firstMatchingBuild(builds,version=''){const unique=[...new Set(builds.filter(Boolean))];if(version){const matching=unique.filter(b=>versionOnly(b)===version);return matching[0]||''}return unique[0]||''}
...
export async function onRequestGet(context){
  ...
  const cacheKey=new Request(new URL('/api/status?cache=v6',context.request.url).toString());
  ...
  const liveVersions=[
    liveStatusVersion(ptuFaqBody),
    liveStatusVersion(ptuInstallBody),
    matrixVersion(loanerBody),
    ...deploymentVersions(statusBody),
    ...buildMatches(statusBody,'live').map(versionOnly),
    ...buildMatches(patchBody,'live').map(versionOnly),
    ...buildMatches(ptuFaqBody,'live').map(versionOnly),
    ...buildMatches(ptuInstallBody,'live').map(versionOnly)
  ];
  const liveVersion=newestVersion(liveVersions);
  const matrixBuild=loanerBody.match(/Last Updated:[^|]{0,120}\|\s*(\d+(?:\.\d+){1,3}-live\.\d+)/i)?.[1]||'';
  const liveBuildCandidates=[...buildMatches(statusBody,'live'),...buildMatches(patchBody,'live'),...buildMatches(ptuFaqBody,'live'),...buildMatches(ptuInstallBody,'live'),matrixBuild];
  ...
  const ptuState=ptuStateFrom(ptuInstallBody)||ptuStateFrom(ptuFaqBody)||(ptuBuilds.length?'Online':'Unknown');
