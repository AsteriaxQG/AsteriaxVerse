...
export async function onRequestGet(context){
  ...
  const cacheKey=new Request(new URL('/api/status?cache=v3',context.request.url).toString());
  ...
  const payload={
    ...
    eptu:{version:versionOnly(eptuBuilds[0]||''),build:eptuBuilds[0]||'',status:'Non publié'},
