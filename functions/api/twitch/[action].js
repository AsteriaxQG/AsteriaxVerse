...
export async function onRequest(ctx){
  ...
  try{
    ...
    if(action==='callback'&&request.method==='GET'){
      ...
      const raw=random(),age=30*86400,old=readCookie(request,SESSION_COOKIE);
      const tokens=JSON.stringify({accessToken:t.access_token,refreshToken:t.refresh_token||'',accessExpires:now()+Math.min(t.expires_in,v.expires_in)});
      await env.AX_DB.batch([
        env.AX_DB.prepare('DELETE FROM ax_sessions WHERE hash=?').bind(await hash(old)),
        env.AX_DB.prepare('INSERT INTO ax_sessions(hash,user_id,login,token,expires) VALUES(?,?,?,?,?)').bind(await hash(raw),v.user_id,v.login,await seal(tokens,env),now()+age)
      ]);
      return redirect(ORIGIN+'/?twitch=connected#hangar',[cookie(STATE_COOKIE,'',0),cookie(SESSION_COOKIE,raw,age)]);
    }
    if(action==='session'&&request.method==='GET'){
      const s=await session(request,env),r=json({configured:true,user:s?{id:s.id,login:s.login}:null});
      if(s)r.headers.set('Set-Cookie',cookie(SESSION_COOKIE,s.raw,30*86400));
      return r;
    }
