...
export async function session(request,env){
  ...
  const stored=await unseal(s.token,env);let bundle;
  try{bundle=JSON.parse(stored)}catch{bundle={accessToken:stored,refreshToken:'',accessExpires:0}}
  if(!bundle?.accessToken)return null;
  let v=null;
  if(Number(bundle.accessExpires)>now()+300)v={user_id:s.user_id,login:s.login};
  else{
    v=await validate(bundle.accessToken,env);
    if((!v||v.expires_in<=300)&&bundle.refreshToken){
      const refreshed=await tokenRequest(env,{grant_type:'refresh_token',refresh_token:bundle.refreshToken});
      v=await validate(refreshed.access_token,env);
      if(v?.user_id===s.user_id){
        bundle={accessToken:refreshed.access_token,refreshToken:refreshed.refresh_token||bundle.refreshToken,accessExpires:now()+Math.min(refreshed.expires_in,v.expires_in)};
        await env.AX_DB.prepare('UPDATE ax_sessions SET token=?,expires=? WHERE hash=?').bind(await seal(JSON.stringify(bundle),env),now()+30*86400,id).run();
      }
    }
  }
  if(!v||v.user_id!==s.user_id){await env.AX_DB.prepare('DELETE FROM ax_sessions WHERE hash=?').bind(id).run();return null}
  await env.AX_DB.prepare('UPDATE ax_sessions SET expires=? WHERE hash=?').bind(now()+30*86400,id).run();
  return{id:s.user_id,login:s.login,sessionHash:id,raw,token:bundle.accessToken};
}
