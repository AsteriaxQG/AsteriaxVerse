(()=>{
  ...
  function paint(){
    ...
    document.querySelectorAll('[data-changelog-label]').forEach(el=>el.textContent='Patch Notes');
    ...
    if(!list)return;
    document.title='Asteriax Verse — Patch Notes';
    document.querySelector('#releaseTitle').textContent='Patch Notes';
    ...
    if(!versions.length){list.append(text('p',failed?(english?'Patch Notes unavailable. Please reload to retry.':'Patch Notes indisponibles. Recharge la page pour réessayer.'):(english?'Loading…':'Chargement…')));return}
