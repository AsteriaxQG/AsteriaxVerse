...
function sortGrid(grid,type,kind){if(!grid)return;const cards=[...grid.children];cards.sort((a,b)=>{const name=x=>(x.querySelector('h3')?.textContent||'').trim();const price=x=>num(x.querySelector('.price')?.textContent);if(type==='price')return price(a)-price(b);if(type==='price-desc')return price(b)-price(a);if(type==='scu'){const vals=x=>[...x.querySelectorAll('.card-meta>div')].find(d=>d.querySelector('span')?.textContent.trim()==='SCU')?.querySelector('strong')?.textContent;return num(vals(b))-num(vals(a))}return name(a).localeCompare(name(b),'fr',{numeric:true})});cards.forEach(c=>grid.appendChild(c))}
function bindSort(id,grid,kind){const s=$(id),g=$(grid);if(!s||!g)return;const run=()=>sortGrid(g,s.value,kind);s.addEventListener('change',run);new MutationObserver(()=>{if(!g.dataset.sorting){g.dataset.sorting='1';requestAnimationFrame(()=>{run();delete g.dataset.sorting})}}).observe(g,{childList:true})}
bindSort('#vehicleSort','#vehicleGrid','vehicle');bindSort('#itemSort','#itemGrid','item');
...
// Met à jour le titre de l'onglet selon la section ouverte.
...
// Raccourcis clavier : / place immédiatement le curseur dans la recherche globale.
