...
function pledgePurchase(v){const collector=v?.pledge_collector===true||/^aurora mk i(?!i)/.test(normName(v?.name));return collector?'Indisponible · Collector':v?.pledge_available===true?'Disponible':v?.pledge_available===false?'Indisponible actuellement':'Non vérifié'}
