...
function pledgePrice(v){const n=Number(v?.pledge_price);if(v?.pledge_available===false)return'Indisponible actuellement';if(v?.pledge_available!==true||!Number.isFinite(n)||n<=0)return'Non vérifié';const amount=n.toLocaleString('fr-FR',{minimumFractionDigits:2,maximumFractionDigits:2});return`${amount} $ US${v?.pledge_is_warbond?' · Warbond':''}`}
...
async function loadCatalog(){try{
 ...
 for(const c of data.items){
  ...
  if(v){
   Object.assign(v,{name:cname,name_full:cname,manufacturer:manufacturer||human(v.manufacturer),production_status:status,in_game:c.in_game===true,is_concept:/concept/i.test(status)?1:0,catalog_type:human(c.type),catalog_size:human(c.size),catalog_length:human(c.length)||human(v.catalog_length),catalog_beam:human(c.beam),catalog_height:human(c.height),catalog_mass:human(c.mass),catalog_description:human(c.description),catalog_image:human(c.image)||human(v.url_photo),rsi_url:human(c.rsi_url),store_url:human(c.store_url),pledge_available:c.pledge_available===true?true:c.pledge_available===false?false:null,pledge_price:c.pledge_price,pledge_currency:human(c.pledge_currency),pledge_is_warbond:c.pledge_is_warbond===true,pledge_discounted:c.pledge_discounted===true,roles:human(c.focus)||human(v.roles),scu:c.scu??v.scu,crew_min:c.crew_min,crew:c.crew_max??v.crew,msrp:c.msrp});
   merged.push(v);
  }else{
   merged.push({id:`rsi-${human(c.rsi_id)||matchKey}`,name:cname,name_full:cname,manufacturer,production_status:status,in_game:c.in_game===true,is_concept:/concept/i.test(status)?1:0,catalog_type:human(c.type),catalog_size:human(c.size),catalog_length:human(c.length),catalog_beam:human(c.beam),catalog_height:human(c.height),catalog_mass:human(c.mass),catalog_description:human(c.description),catalog_image:human(c.image),rsi_url:human(c.rsi_url),store_url:human(c.store_url),pledge_available:c.pledge_available===true?true:c.pledge_available===false?false:null,pledge_price:c.pledge_price,pledge_currency:human(c.pledge_currency),pledge_is_warbond:c.pledge_is_warbond===true,pledge_discounted:c.pledge_discounted===true,roles:human(c.focus),scu:c.scu,crew_min:c.crew_min,crew:c.crew_max,msrp:c.msrp,is_ground_vehicle:human(c.size).toLowerCase()==='vehicle'||human(c.type).toLowerCase()==='ground'?1:0,price_min:null,location:'',catalog_only:true});
  }
 }
