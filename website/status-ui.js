(()=>{
  ...
  function setEnv(prefix,data,label){let status=data?.status||((prefix==='Ptu'||prefix==='Eptu'||prefix==='Hotfix')?'Hors ligne':'Non publié');if(prefix==='Live'&&norm(status)==='operationnel')status='En ligne';if((prefix==='Ptu'||prefix==='Eptu')&&norm(status)==='non verifie')status='Non disponible';const version=data?.version?`Alpha ${data.version}`:'—';const build=data?.build||label;const statusEl=q(`#home${prefix}Status`),versionEl=q(`#home${prefix}Version`),buildEl=q(`#home${prefix}Build`);if(statusEl){statusEl.textContent=status;decorate(statusEl,status)}if(versionEl)versionEl.textContent=version;if(buildEl){buildEl.textContent=build;buildEl.title=build}if(prefix==='Hotfix'){const card=statusEl?.closest('.verse-env');if(card)card.hidden=norm(status)!=='en ligne'}}
