(()=>{
  ...
  function setEnv(prefix,data,label){const status=data?.status||'Non publié';const version=data?.version?`Alpha ${data.version}`:'—';const build=data?.build||label;const statusEl=q(`#home${prefix}Status`),versionEl=q(`#home${prefix}Version`),buildEl=q(`#home${prefix}Build`);if(statusEl){statusEl.textContent=status;decorate(statusEl,status)}if(versionEl)versionEl.textContent=version;if(buildEl){buildEl.textContent=build;buildEl.title=build}}
