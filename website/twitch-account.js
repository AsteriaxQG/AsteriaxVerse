(()=>{
  ...
  const login=document.createElement('a');login.id='axLogin';login.className='ax-top-login';login.href='/api/twitch/login';login.hidden=true;headerActions.append(login);document.querySelector('.nav')?.insertAdjacentElement('afterend',headerActions);
  const adoptLanguageToggle=()=>{const toggle=document.querySelector('.language-toggle');if(toggle&&toggle.parentElement!==headerActions)headerActions.prepend(toggle)};
